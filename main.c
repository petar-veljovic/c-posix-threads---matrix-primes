...
int ext_threads()
{
  ...
  for (i = 0; i < NUM_THREADS; i++) { result_code = pthread_join(threads[i], NULL);}
...
{
  if (n == 1) { return 0;}
  // trecho comentado para melhor demonstracao do pthreads
  /*
  if (round(n / 2) == n / 2)   { return 1;}
  if (round(n / 3) == n / 3)   { return 1;}
  if (round(n / 5) == n / 5)   { return 1;}
  if (round(n / 7) == n / 7)   { return 1;}
  if (round(n / 10) == n / 10) { return 1;}
  if (round(n / 11) == n / 11) { return 1;}
  */
  int raiz = sqrt(n);
  for (int i = 2; i <= raiz; i++) { if (n % i == 0) { return 0; } }
  return 1;
}
...
{
  free(vetIndexMacro);
  for (int i = 0; i < M_LINHA; i++) { free(matriz[i]); }
  free(matriz);
}